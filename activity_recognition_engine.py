...
class ActivityRecognitionEngine(cognitive_engine.Engine):
    ...
    def run_act_detector(self):
        assert len(tracker.frame_history) >= self.tracker.timesteps, (
            'Tracker was not run on all {} frames'.format(tracker.timesteps))
        if not tracker.active_actors:
